...
async def get_andopt2_products():
    async with SupplierAndopt2(email=EMAIL, password=PASSWORD) as parser:
...
async def get_dtopelectronic_products():
    async with SupplierDtopelectronic(email=EMAIL, password=PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_jmaxtvshop_products():
    async with SupplierJmaxtvshop(email=EMAIL, password=PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_venera7km_products():
    async with SupplierVenera7km(email=EMAIL, password=PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_grantopt_products():
    async with SupplierGrantopt(email=EMAIL, password=PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_melad_products():
    async with SupplierMelad(email=EMAIL, password=MELAD_PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_jumpex_products():
    async with SupplierJumpex(email=EMAIL, password=JUMPEX_PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
...
async def get_matrix7km_products():
    async with SupplierMatrix7km(email=EMAIL, password=PASSWORD) as parser:
        products = await parser.parse_all()
    return JSONResponse(content=[p.model_dump(mode="json") for p in products])
