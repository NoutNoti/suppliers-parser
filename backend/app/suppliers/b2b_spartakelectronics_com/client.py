        ...
        async def process_category(data):
            ...
            while True:
                ...
                try:
                    ...
                    logging.warning("Timeout for %s page %s", name, page)
                    break
                ...
                tasks = [self._extract_product(row, name) for row in product_rows]
    ...
    async def _extract_product(self, block: Tag, category_name: str) -> ExtractedProduct | None:
        ...
        for cell in cells:
            ...
            if "шт" in cell_text.lower():
                qty_match = re.search(r"(\d+)\s*шт", cell_text, re.IGNORECASE)
                if qty_match:
        ...
        if len(cells) > 6:
            ...
            if detail_link:
                ...
                m = re.search(r"/catalog/product/(\d+)", href)
                if m:
