...
class SupplierJumpex(BaseSupplierParser):
    ...
    async def _login(self):
        ...
        data = {"username": self.email, "passwd": self.password}
        if csrf_token:
            data[csrf_token] = "1"

        resp = await self.client.post(f"{self.base_url}/user/loginsave", data=data)
    ...
    async def _get_all_categories(self, category_tag: str = None) -> list:
        ...
        all_urls = {}
        skip_words = ["login", "logout", "about", "delivery", "contacts",
                       "search", "registration", "recover", "cart", "user",
                       "wishlist", "orders"]
        ...
        for a in links:
            ...
            if url not in all_urls:
                all_urls[url] = {"url": url, "name": name, "depth": len(segments)}
        ...
        for url, data in all_urls.items():
            if data["depth"] == 2:
                categories.append({"url": url, "name": data["name"]})
            elif data["depth"] == 1 and url.rstrip("/") not in parent_urls:
                categories.append({"url": url, "name": data["name"]})

        return categories

    async def _extract_product(self, block: Tag, category_name: str) -> ExtractedProduct | None:
