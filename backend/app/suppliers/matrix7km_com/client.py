...
class SupplierMatrix7km(BaseSupplierParser):
    ...
    async def _get_all_categories(self, category_tag: str = None) -> list:
        ...
        all_urls = {}
        skip_words = ["login", "logout", "account", "contact", "index.php",
                       "wishlist", "compare", "information", "special"]
        ...
        for a in links:
            ...
            path = url.replace(f"{self.base_url}/", "").replace("ua/", "")
    ...
    async def _extract_product(self, block: Tag, category_name: str) -> ExtractedProduct | None:
        ...
        if stock_text:
            ...
            stock_status = StockStatus.OUT_OF_STOCK if no_stock_el else StockStatus.UNKNOWN
